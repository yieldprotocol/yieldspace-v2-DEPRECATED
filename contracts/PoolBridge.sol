...
contract PoolBridge {
    ...
    /// @param src Pool
    /// @param poolTokenIn Amount of pool tokens
    ...
    function burn(address src, uint256 poolTokenIn, uint256 minBaseTokenOut, uint256 minFYTokenOut)
        external
        returns (uint256, uint256, uint256)
    {
        IERC20(src).transferFrom(msg.sender, src, poolTokenIn);
    ...
    /// The trader needs to have called `src.approve`
    /// @param src Pool
    /// @param poolTokenBridged The amount of pool tokens
    ...
    function bridge(address src, address dst, uint256 poolTokenBridged, uint256 fyTokenToBuy, uint256 minTokensMinted)
        external
        returns (uint256, uint256, uint256)
    {
        IERC20(src).transferFrom(msg.sender, src, poolTokenBridged);
