...
contract('Pool', async (accounts) => {
  ...
  it('should setup pool', async () => {
    ...
    expect(await pool1.k()).to.be.bignumber.equal(k)
  ...
  it('adds initial liquidity', async () => {
    ...
    await dai.approve(pool1.address, MAX, { from: user1 })
    const tx = await pool1.init(initialDai, { from: user1 })
  ...
  describe('with initial liquidity', () => {
    beforeEach(async () => {
      ...
      await pool1.init(initialDai, { from: user1 })
    })

    it('sells fyDai', async () => {
      const daiReserves = await pool1.getDaiReserves()
      const fyDaiReserves = await pool1.getFYDaiReserves()
      ...
      const daiOutPreview = await pool1.sellFYDaiPreview(fyDaiIn, { from: operator })
      ...
      await pool1.addDelegate(operator, { from: from })
      await fyDai1.mint(from, fyDaiIn, { from: owner })
      await fyDai1.approve(pool1.address, fyDaiIn, { from: from })
      const tx = await pool1.sellFYDai(from, to, fyDaiIn, { from: operator })
    ...
    it('buys dai', async () => {
      ...
      // Test preview since we are here
      const fyDaiInPreview = await pool1.buyDaiPreview(daiOut, { from: operator })
      ...
      await pool1.addDelegate(operator, { from: from })
      await fyDai1.approve(pool1.address, fyDaiTokens, { from: from })
      const tx = await pool1.buyDai(from, to, daiOut, { from: operator })
    ...
    describe('with extra fyDai reserves', () => {
      beforeEach(async () => {
        ...
        await fyDai1.approve(pool1.address, initialFYDai, { from: operator })
        await pool1.sellFYDai(operator, operator, initialFYDai, { from: operator })
      })

      it('mints liquidity tokens', async () => {
        const oneToken = toWad(1)
        const daiReserves = await dai.balanceOf(pool1.address)
        const fyDaiReserves = await fyDai1.balanceOf(pool1.address)
        const supply = await pool1.totalSupply()
        ...
        const poolTokensBefore = await pool1.balanceOf(user2)

        await dai.approve(pool1.address, MAX, { from: user1 })
        await fyDai1.approve(pool1.address, MAX, { from: user1 })
        const tx = await pool1.mint(user1, user2, fyDaiTokens, 0, MAX, { from: user1 })
        ...
        const minted = (await pool1.balanceOf(user2)).sub(poolTokensBefore)
      ...
      it('mints liquidity tokens with dai and fyDai, buying FYDai', async () => {
        const oneToken = toWad(1)
        const daiReserves = await dai.balanceOf(pool1.address)
        const fyDaiReservesVirtual = await pool1.getFYDaiReserves()
        const fyDaiReservesReal = await fyDai1.balanceOf(pool1.address)
        const supply = await pool1.totalSupply()
        ...
        await dai.approve(pool1.address, MAX, { from: user1 })
        await fyDai1.approve(pool1.address, MAX, { from: user1 })
        const tx = await pool1.mint(user1, user2, fyDaiIn, fyDaiToBuy, MAX, { from: user1 })
      ...
      it('mints liquidity tokens with dai and fyDai, selling FYDai', async () => {
        ...
        await dai.approve(pool1.address, MAX, { from: user1 })
        await fyDai1.approve(pool1.address, MAX, { from: user1 })
        const tx = await pool1.mint(user1, user2, fyDaiIn, fyDaiToBuy.neg(), MAX, { from: user1 })
      ...
      it('mints liquidity tokens with dai only, buying FYDai', async () => {
        ...
        await dai.approve(pool1.address, MAX, { from: user1 })
        // await fyDai1.approve(pool1.address, MAX, { from: user1 })
        const tx = await pool1.mint(user1, user2, 0, fyDaiToBuy, MAX, { from: user1 })
      ...
      it('burns liquidity tokens', async () => {
        ...
        await pool1.approve(pool1.address, lpTokensIn, { from: user1 })
        const tx = await pool1.burn(user1, user2, lpTokensIn, { from: user1 })
        ...
        const daiOut = daiReserves.sub(await dai.balanceOf(pool1.address))
        const fyDaiOut = fyDaiReserves.sub(await fyDai1.balanceOf(pool1.address))
      ...
      it('burns liquidity tokens to Dai', async () => {
        ...
        await pool1.approve(pool1.address, lpTokensIn, { from: user1 })
        const tx = await pool1.burnAndSellFYDai(user1, user2, lpTokensIn, { from: user1 })
      ...
      it('sells dai', async () => {
        ...
        // Test preview since we are here
        const fyDaiOutPreview = await pool1.sellDaiPreview(daiIn, { from: operator })
        ...
        await dai.approve(pool1.address, daiIn, { from: from })

        const tx = await pool1.sellDai(from, to, daiIn, { from: operator })
      ...
      it('buys fyDai', async () => {
        ...
        // Test preview since we are here
        const daiInPreview = await pool1.buyFYDaiPreview(fyDaiOut, { from: operator })
        ...
        await dai.approve(pool1.address, daiTokens, { from: from })
        const tx = await pool1.buyFYDai(from, to, fyDaiOut, { from: operator })
      ...
      it("once mature, doesn't allow trading", async () => {
        ...
        await expectRevert(pool1.sellDaiPreview(oneToken, { from: operator }), 'Pool: Too late')
        await expectRevert(pool1.sellDai(from, to, oneToken, { from: from }), 'Pool: Too late')
        await expectRevert(pool1.buyDaiPreview(oneToken, { from: operator }), 'Pool: Too late')
        await expectRevert(pool1.buyDai(from, to, oneToken, { from: from }), 'Pool: Too late')
        await expectRevert(pool1.sellFYDaiPreview(oneToken, { from: operator }), 'Pool: Too late')
        await expectRevert(pool1.sellFYDai(from, to, oneToken, { from: from }), 'Pool: Too late')
        await expectRevert(pool1.buyFYDaiPreview(oneToken, { from: operator }), 'Pool: Too late')
        await expectRevert(pool1.buyFYDai(from, to, oneToken, { from: from }), 'Pool: Too late')
      })
    })
  })
})
