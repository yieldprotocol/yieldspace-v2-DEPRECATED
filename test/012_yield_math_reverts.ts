...
import { secondsInOneYear, secondsInTenYears, ts, g0 } from '../src/constants'
...
describe('YieldMath - Reverts', async function () {
  ...
  describe('fyTokenOutForBaseIn reverts', () => {
    ...
    // If time to maturity is higher than 1/ts, multiplied or divided by g, we are too far from maturity.
    ...
    it('Too much base in', async () => {
      await expect(yieldMath.fyTokenOutForBaseIn(MAX, WAD.mul(10), WAD, secondsInOneYear, ts, g0)).to.be.revertedWith(
    ...
    it('Insufficient fyToken balance', async () => {
      await expect(
        yieldMath.fyTokenOutForBaseIn(WAD, WAD.mul(10), WAD.mul(20), secondsInOneYear, ts, g0)
      ).to.be.revertedWith('YieldMath: Insufficient fyToken reserves')
    })

    /* it("Rounding induced error", async () => {
      await expect(
        yieldMath.fyTokenOutForBaseIn(WAD, WAD, 0, secondsInOneYear, ts, g0)
      ).to.be.revertedWith(
  ...
  describe('baseOutForFYTokenIn reverts', () => {
    ...
    it('Too much fyToken in', async () => {
      await expect(yieldMath.baseOutForFYTokenIn(WAD.mul(10), MAX, WAD, secondsInOneYear, ts, g0)).to.be.revertedWith(
    ...
    it('Insufficient base balance', async () => {
      await expect(
        yieldMath.baseOutForFYTokenIn(WAD.mul(10), WAD, WAD.mul(20), secondsInOneYear, ts, g0)
      ).to.be.revertedWith('YieldMath: Insufficient base reserves')
    })

    /* it("Rounding induced error", async () => {
      await expect(
        yieldMath.baseOutForFYTokenIn(MAX, WAD, WAD, 1, ts, g0)
      ).to.be.revertedWith(
        'YieldMath: Rounding induced error'
      )
    }) */
  ...
  describe('fyTokenInForBaseOut reverts', () => {
    ...
    it('Too much base out', async () => {
      await expect(
        yieldMath.fyTokenInForBaseOut(WAD.mul(2), WAD, WAD.mul(3), secondsInOneYear, ts, g0)
    ...
    it('Resulting fyToken balance too high', async () => {
      await expect(
        yieldMath.fyTokenInForBaseOut(WAD.mul(10), MAX, WAD, secondsInOneYear.mul(4), ts, g0),
        'YieldMath: Resulting fyToken balance too high'
      ).to.be.revertedWith('YieldMath: Resulting fyToken reserves too high')
    })

    it('Rounding induced error', async () => {
      await expect(yieldMath.fyTokenInForBaseOut(WAD.mul(10), MAX, WAD, 1, ts, g0)).to.be.revertedWith(
        'YieldMath: Rounding induced error'
      )
    })
  })
  ...
  describe('baseInForFYBaseOut reverts', () => {
    ...
    it('Too much fyToken out', async () => {
      await expect(yieldMath.baseInForFYTokenOut(WAD, WAD, WAD.mul(2), secondsInOneYear, ts, g0)).to.be.revertedWith(
    ...
    it('Resulting base balance too high', async () => {
      await expect(
        yieldMath.baseInForFYTokenOut(MAX, WAD.mul(10), WAD, secondsInOneYear.mul(4), ts, g0)
      ).to.be.revertedWith('YieldMath: Resulting base reserves too high')
    })

    it('Rounding induced error', async () => {
      await expect(
        yieldMath.baseInForFYTokenOut(MAX, WAD, WAD, 1, ts, g0) // Why does it revert? No idea.
